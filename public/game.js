...
// Adicionado 'animTick' para controlar a velocidade da onda do pulinho
    ...
    ws.onmessage = (event) => {
        ...
        if (dados.tipo === "novo_jogador") {
            if (dados.username !== meuBicho.username) {
                dados.chatTexto = ""; dados.chatTimer = 0; dados.isTyping = false; dados.lado = dados.lado || "direita"; dados.animTick = 0;
            ...
            dados.jogadores.forEach(p => {
                if (p.username !== meuBicho.username) {
                    p.chatTexto = ""; p.chatTimer = 0; p.isTyping = false; p.lado = p.lado || "direita"; p.animTick = 0;
            ...
            if (outrosJogadores[dados.id]) {
                ...
                outrosJogadores[dados.id].lado = dados.lado || "direita";
            }
        } 
...
function atualizarFisica() {
    ...
    if (teclas['ArrowLeft'] || teclas['a']) { dx -= 1; meuBicho.lado = "esquerda"; }
    if (teclas['ArrowRight'] || teclas['d']) { dx += 1; meuBicho.lado = "direita"; }
    ...
    if (dx !== 0 || dy !== 0) {
        ...
        // Se está se movendo, acumula o tick da animação de pulo
        ...
        // Se parou, reseta o pulo pro bicho assentar no chão
        meuBicho.animTick = 0;
    }
...
// ==========================================
// RENDERIZADORES COM FLIP CORRIGIDO (QoL)
// ==========================================
function desenharSpriteInvertido(img, x, y, tamanho, lado) {
    // CORRIGIDO: Agora flipa se estiver indo para a DIREITA! Ajustado para casar com a arte
    if (lado === "direita") {
        ctx.save();
...
function desenharReguaDebug() {
    ctx.save();
    ctx.strokeStyle = "rgba(255, 255, 255, 0.12)"; ctx.lineWidth = 0.5; ctx.font = "7px monospace"; ctx.fillStyle = "rgba(255, 255, 255, 0.4)";
    ctx.textAlign = "left";
...
function desenhar() {
    ...
    // ==========================================
    // JUICE: RENDER DOS OUTROS PLAYERS
    // ==========================================
    for (let id in outrosJogadores) {
        let p = outrosJogadores[id];
        let imgSpriteOutro = imagensSprites[p.spriteId];

        // Descobre se o outro player se moveu calculando o delta de frames local
        let seMoveuOutro = (p.x !== p.xAnterior || p.y !== p.yAnterior);
        if (seMoveuOutro) {
            p.animTick = (p.animTick || 0) + 0.25;
        } else {
            p.animTick = 0;
        }
        p.xAnterior = p.x; p.yAnterior = p.y; // Atualiza rastro de rampa

        // Aplica a modulação elástica senoidal de pulo se o bicho estiver andando
        ...
        ctx.fillStyle = "white"; ctx.fillText(p.username, p.x + (meuBicho.tamanho/2), p.y - 5 + bobeioYOutro);
    ...
    // ==========================================
    // JUICE: RENDER DO SEU BICHO LOCAL
    // ==========================================
    // Calcula o deslocamento vertical do pulo baseado no movimento
    ...
    ctx.fillStyle = "white"; ctx.fillText(meuBicho.username, meuBicho.x + (meuBicho.tamanho/2), meuBicho.y - 5 + bobeioYMeu);
